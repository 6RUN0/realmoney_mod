...
$plexprice = config::get('real_money_mod_plexprice'); if($plexprice == 0) { config::set('real_money_mod_plexprice', '34.99'); $plexprice = config::get('real_money_mod_plexprice'); }
$faktorEuro = config::get('real_money_mod_euro'); if($faktorEuro == 0) { config::set('real_money_mod_euro', '1.3'); $faktorEuro = config::get('real_money_mod_euro'); }
$faktorPfund = config::get('real_money_mod_pfund'); if($faktorPfund == 0) { config::set('real_money_mod_pfund', '1.56'); $faktorPfund = config::get('real_money_mod_pfund'); }
$versionanzeige = config::get('real_money_mod_ver');

$html .= "
   <form name=\"add\" action=\"?a=settings_realmoney_mod&amp;step=add\" method=\"post\">
   <table width=\"75%\">
    <tr><td>Current GTC price in Dollar:</td><td><input type=\"text\" name=\"add_plexprice\" value=\"".$plexprice."\" /></td></tr>
    <tr><td>Exchange ratio Euro &raquo; USD:</td><td><input type=\"text\" name=\"add_faktorEuro\" value=\"".$faktorEuro."\" /></td></tr>
    <tr><td>Exchange ratio GBP (Pound) &raquo; USD:</td><td><input type=\"text\" name=\"add_faktorPfund\" value=\"".$faktorPfund."\" /></td></tr>
